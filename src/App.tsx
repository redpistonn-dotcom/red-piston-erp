/**
 * App.jsx — root orchestrator for the Shop app (red-piston-shop).
 *
 * This is the owner/staff/mechanic-facing split of the original single-SPA
 * red-piston-frontend — carries ERPShell + MechanicShell + the car-decor
 * management pages only. MPShell/AdminShell and the marketplace/admin routes
 * live in their own (not-yet-split) apps, not here.
 *
 * WHAT LIVES HERE:
...
class PageErrorBoundary extends Component {
  ...
  render() {
    ...
    // Forward extra props to the child (e.g. a page needing shell-provided context)
...
// (Shop app only ever has SHOP_OWNER/SHOP_STAFF/MECHANIC users — no CUSTOMER
// branch here, that lives in the marketplace app.)
...
function AppContent() {
  ...
  const appCtxValue = useMemo(() => ({
    ...
    // Auth (consumed by shells + the sign-in page)
  ...
  return (
    <AppCtx.Provider value={appCtxValue}>
      ...
      <Suspense fallback={<PageLoader />}>
        <Routes>
          {/* "/" is the sign-in screen for unauthenticated users — Shop Owner tab
              pre-selected, since this app's audience is owner/staff/mechanic. */}
          <Route path="/" element={currentUser ? <Navigate to={getDefaultRoute(currentUser)} replace /> : <LoginPage onLogin={handleLogin} defaultTab="owner" />} />
